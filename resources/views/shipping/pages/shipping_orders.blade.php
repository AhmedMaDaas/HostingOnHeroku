          ...
          <table class="table card-table table-responsive table-responsive-xl orders-table" style="width:100%">
            ...
            <tbody>
              @foreach($bills as $bill)
                <tr id="{{ $bill->id }}" class="{{ $bill->new ? 'new-order' : '' }}">
        ...
        <div class="card-body">
          <nav aria-label="Page navigation example">
            <ul class="pagination pagination-flat pagination-flat-rounded">
              <li class="page-item">
                <a class="page-link" href="{{ $page > 1 ? url('shipping/shipping-orders') . '?page=' . ($page - 1) : '#' }}" aria-label="Previous">
              ...
              <li class="page-item {{ $page == $i ? 'active' : '' }}">
                <a class="page-link" href="{{ url('shipping/shipping-orders') }}?page={{ $i }}">{{ $i }}</a>
              </li>
              @endfor
              <li class="page-item">
                <a class="page-link" href="{{ $page == $pagesNumber ? '#' : url('shipping/shipping-orders') . '?page=' . ($page + 1) }}" aria-label="Next">
...
@push('js')
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      $(document).on('click', '.accept-order', function(e){
        e.preventDefault();
        var id = $(this).attr('id');
        var url = "{{ url('shipping/accept-order') }}";
        var type = 'post';
      ...
      $(document).on('click', '.update-shipping-coast', function(e){
        e.preventDefault();
        var id = $(this).attr('id');
        var url = "{{ url('shipping/update-order') }}";
        var type = 'post';
      ...
      $(document).on('click', '.reject-order', function(e){
        e.preventDefault();
        var id = $(this).attr('id');
        var url = "{{ url('shipping/reject-order') }}";
        var type = 'post';
        orderAction(id, url, type, $(this));
      });
      ...
      $(document).on('click', '.customer-name', function(){
        var bill = $(this).parent('td').parent('tr');
        var id = bill.attr('id');
        if(bill.hasClass('new-order')){
          bill.removeClass('new-order');
          makeOld(id);
        }
      });
  ...
  <script>
    ...
    function getNewOrder(){
      $.ajax({
        url: "{{ url('/') }}/shipping/shipping-orders",
        type: 'get',
        data: {},
        success: function(data){
